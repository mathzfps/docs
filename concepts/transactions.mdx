...
stateDiagram-v2
    ...
    processing --> refunded: banco rejeitou (wallet estornada)
```
...
| `refunded` | ✅ | Banco rejeitou (ex: chave inválida). **Wallet estornada automaticamente.** |
| `failed` | ✅ | Erro local antes do envio (saldo insuficiente, validação). Wallet nunca foi debitada. |
